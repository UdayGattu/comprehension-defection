...
`.gitignore` records the archives as **~868 MB in total and already in git
history**; they are deliberately not ignored, because ignoring them would do
...
153 test functions across 12 files; **314 test cases** once parametrisation is
...
analysis/           thirteen scripts, numbered 01-04 and 06-14 (there is no 05;
                    the numbering is historical). All read-only over the
                    databases, all open them mode=ro&immutable=1.
scripts/            six experiment drivers, exp2_mechanism.sh through
                    exp7_confounds.sh, plus session1.sh from an earlier
                    session; the GPU runner (gpu_run.py); reproduce.sh; and
                    the CPU instrument checks (smoke_test.py,
                    tokenizer_check.py, calibrate_block.py, pilot.py,
                    preregister.py)
