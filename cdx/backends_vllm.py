...
# Appended AFTER the reasoning; the next token is the one scored.
#
# THE FORMAT DEMAND BELONGS HERE, NOT IN THE INSTRUCTION.
#     Putting "exactly one word" in the instruction suppressed reasoning
#     outright - Qwen2.5 answered and stopped. Putting it here cannot, because
#     the reasoning has already been generated by the time this is appended.
#
#     Without it, Qwen ran at ~100% off-task: its reasoning is verbose markdown
#     that truncates mid-structure at the token budget, and a bare
#     "Final answer:" left it wanting to continue the list rather than answer.
#     Llama, whose reasoning ends in prose, was unaffected - so the weak cue
#     worked for one model and silently failed for another.
#
# Deliberately names no action, for the same reason the instruction does not:
# a label here would be a lexical cue, and lexical cues are the effect under
# study.
SCRATCHPAD_CUE = "\n\nFinal answer, exactly one word:"
...
class VLLMBackend:
    ...
    def _decide_scratchpad(self, prompts, seeds, framing) -> list[Decision]:
        ...
        cue = self.tokenizer.encode(SCRATCHPAD_CUE)
        followups = [
